    ...
    def _combine_analysis(self, analysis_results: Dict, symbol: str, onchain_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if confidence >= 0.8:
                ...
                base_leverage = 5.0
                if confidence >= 0.96:
                    # Экстремально высокая уверенность (96-97%) - максимальное плечо 50x
...
class UnifiedSignalBot:
    ...
    def __init__(self):
        ...
        self.scalping_engine = ScalpingSignalEngine(min_confidence=0.6, min_filters=3)  # Настройки для скальпинга  # ЗОЛОТАЯ СЕРЕДИНА: Баланс качества и частоты
        self.running = False
        self.start_time = time.time()  # Добавляем для отслеживания времени работы
        
        # Конфигурация
        self.pairs = TRADING_PAIRS
        self.timeframes = ['15m', '1h', '4h']  # Основные таймфреймы для обычных сигналов  # Добавляем 1m для скальпинга
    ...
    async def start(self):
        ...
        await self.telegram_bot.send_message(
            ...
            "🎯 SignalPro: Обычные сигналы (80%+ уверенность)"
            "ScalpingPro: Быстрые сигналы (60%+ уверенность)\n\n"
