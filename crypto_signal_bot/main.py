    ...
    def get_binance_data(symbol: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"📊 Binance raw data: {data}")
                
                price = float(data['lastPrice'])
                change_24h = float(data['priceChangePercent'])
                volume = float(data['volume'])
                
                print(f"💰 {symbol} - Price: ${price}, Change: {change_24h}%, Volume: {volume}")
...
class AIAnalyzer:
    ...
    def __init__(self):
        self.thresholds = {
            'strong_buy': 0.6,  # Было 0.85, теперь 0.6
            'buy': 0.4,         # Было 0.65, теперь 0.4
            'neutral': 0.3,     # Было 0.35, теперь 0.3
            'sell': 0.2,        # Было 0.15, теперь 0.2
            'strong_sell': 0.1  # Было 0.05, теперь 0.1
        }
    ...
    def analyze_market_data(self, exchange_data: Dict) -> Dict:
        ...
        print(f"📊 AI Analysis - Avg Price: ${avg_price:.2f}, Change: {avg_change:.2f}%, Volume: {avg_volume:.0f}")
        ...
        print(f"🧠 AI Metrics - Trend Strength: {trend_strength:.3f}, Volatility: {volatility:.3f}, Confidence: {confidence:.3f}")
        
        # Определение сигнала
        signal = self._classify_signal(confidence, avg_change)
        
        print(f"🎯 AI Signal: {signal} (Confidence: {confidence:.3f}, Thresholds: {self.thresholds})")
